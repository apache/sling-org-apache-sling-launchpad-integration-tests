...
public class ScriptBuiltinObjectsTest extends RenderingTestBase {
    ...
    public void testEspBuiltinObjects() throws IOException {
        ...
        try {
            ...
            assertTrue(
                    "Content includes currentNode text (" + content + ")",
                    content.contains("currentNode.text:" + testText));
            assertTrue("Content includes sc data (" + content + ")", content.contains("sc:null"));
            assertTrue(
                    "Content includes response data (" + content + ")",
                    content.contains("SlingHttpServletResponse:false"));
        } finally {
