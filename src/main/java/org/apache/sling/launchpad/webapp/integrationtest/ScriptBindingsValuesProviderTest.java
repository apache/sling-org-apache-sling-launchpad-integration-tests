...
public class ScriptBindingsValuesProviderTest extends RenderingTestBase {
    ...
    public void testGenericProvider() throws IOException {
        ...
        try {
            ...
            assertTrue(
                    "Content doesn't include Groovy-specific test text (" + content + ")",
                    content.contains("groovyHelloWorld:undefined"));
        } finally {
    ...
    public void testJSPProvider() throws IOException {
        ...
        try {
            ...
            assertTrue("Content includes test text (" + content + ")", content.contains("Hello World!"));
            assertTrue(
                    "Content includes JSP-specific test text (" + content + ")",
                    content.contains("Hello World from JSP!"));
            assertFalse(
                    "Content doesn't includes Groovy-specific test text (" + content + ")",
                    content.contains("Hello World from Groovy!"));
        } finally {
            testClient.delete(toDelete);
        }
    }
